...
public class S1 {
    public static void main(String[] args) throws IOException {
        ...
        HashSet<Long> beacons = new HashSet<>();
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < scans.get(i).size(); ++j) {
                int x = locs[i][0] + scans.get(i).get(j).get(0);
                int y = locs[i][1] + scans.get(i).get(j).get(1);
                int z = locs[i][2] + scans.get(i).get(j).get(2);
                long num = 42069 - x;
                num <<= 20;
                num += 42069 - y;
                num <<= 20;
